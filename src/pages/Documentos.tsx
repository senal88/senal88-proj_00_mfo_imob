...
import { Documento, Imovel, TIPO_DOCUMENTO_LABELS } from '@/types/imob'
...
export default function Documentos() {
    ...
    try {
      const [docs, imvs] = await Promise.all([
        listarDocumentos({
          ...
          imovelId: imovelFiltro === 'todos' ? undefined : imovelFiltro,
        }),
  ...
  const getNomeImovel = (imovelId: string) => {
    const imv = imoveis.find((i) => i.id === imovelId)
    return imv ? imv.nome : 'Imóvel vinculado'
  }
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">
            Dossiê documental da {usuario?.familia_nome || 'família'} • {documentos.length}{' '}
        ...
        <CardContent className="p-0">
          {loading ? (
            ...
            <div className="p-12 text-center space-y-3">
              ...
              <h3 className="text-base font-bold text-gray-900">Nenhum documento cadastrado</h3>
              <p className="text-xs text-gray-500 max-w-md mx-auto">
                {imovelFiltro !== 'todos'
                  ? 'Não há documentos vinculados ao imóvel selecionado.'
                  : 'Nenhum documento foi vinculado aos imóveis desta família ainda.'}
              </p>
              <Button
                onClick={() =>
                  navigate(
              ...
              <div className="hidden md:block overflow-x-auto">
                <table className="w-full text-left text-xs sm:text-sm">
                  <thead className="bg-gray-50/80 text-gray-600 uppercase text-[11px] font-bold tracking-wider border-b border-gray-200">
                    <tr>
                      <th className="py-3.5 px-6">Nome do documento</th>
                      <th className="py-3.5 px-4">Tipo</th>
                      <th className="py-3.5 px-4">Imóvel vinculado</th>
                      <th className="py-3.5 px-4">Data inclusão</th>
                        ...
                        <td className="py-4 px-6 font-semibold text-gray-900">
                          <div className="flex items-center gap-2.5">
                            ...
                            <div className="min-w-0">
                              <span className="block truncate max-w-xs">{doc.nome}</span>
                        ...
                        <td className="py-4 px-6 text-right">
                          <div className="inline-flex items-center gap-2">
                            <a
                              ...
                              title="Abrir arquivo do storage"
                            >
